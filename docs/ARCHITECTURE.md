...
- 클라이언트가 화면을 누르는 순간 서버에 `throwStart` 메시지를 보낸다. 서버는 그 시각(서버 자체 시계 기준)을 기록하고, 이후 게이지 파형 함수 `gauge(t) = f(elapsed)`(왕복 파형, 구간별 폭은 실제 윷 확률 분포를 반영)를 서버가 소유한다.
- 클라이언트가 손을 떼는 순간 `throwRelease` 메시지를 보낸다. 서버는 `release 시각 - start 시각`을 자신의 시계로 계산해 `gauge()` 함수에 대입, 어느 구간(빽도/도/개/걸/윷/모)에 해당하는지 결정한다.

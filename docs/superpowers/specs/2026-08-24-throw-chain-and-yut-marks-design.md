...
- 한 번의 이동으로 여러 마리를 동시에 잡았다면(업기 스택), **그 스택 전체가 하나의 그룹으로 묶여 함께 부활하거나 함께 그대로 잡힌다**(2026-08-30 변경, v0.35 — 예전엔 스택 각각에 독립적으로 확률을 적용해 일부만 부활하는 어색한 결과가 나왔다). 그룹이 무효화됐으면 그 그룹 전체가 보너스 지급 대상에서 제외되고, 그대로 잡혔으면 그룹 전체가 보너스 지급 대상이다.

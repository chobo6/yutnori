...
**밸런스 주의**: 이 모델은 4개 모서리(5/10/15/출발점)가 전부 대칭(어디서 타든 중앙까지 3칸, 중앙에서 도착까지 3칸)이라고 가정한다 — 실제 윷판은 대각선이 5↔15, 10↔출발점의 두 비대칭 쌍으로 교차하는 구조라 이것과는 다르다. 이 대칭 단순화의 결과로 **15번 모서리에서 지름길을 타면(6칸 소요) 그냥 바깥길로 완주하는 것(15+5=20, 5칸)보다 오히려 손해**다 — 옛 모델에서 15번이 가장 강력한 지름길이었던 것과 정반대로 뒤집혔다. 최종 리뷰에서 지적됐고 아직 사용자에게 확인 전이니, 이 문서를 읽는 세션은 밸런스를 임의로 재조정하지 말고 먼저 사용자에게 확인할 것.
...
- REQUIREMENTS.md 범위 내 기능(서버 엔진, 대기실/플레이 UI, 게이지·Matter.js 던지기 연출, 채팅/말풍선, 캐릭터 능력, 1v1 모드, 지름길 정확 모델)은 전부 구현 완료. 남은 후보는 실제 윷판 모양 시각화(정사각형 트랙 + 캐릭터 표시된 말, 위 "보드 좌표계" 절의 지름길 모델을 기반으로 좌표 렌더링), 15번 모서리 지름길 밸런스 역전 확인(위 "보드 좌표계" 절 참고, 사용자 확인 대기 중), 배포 방식 결정(REQUIREMENTS.md §11) 정도.

...
- **턴 제한시간**(2026-08-21~, REQUIREMENTS.md §4.1, v0.6에서 5초→10초로 상향): 던지기 10초 + 말 선택 10초, 각각 독립적으로 카운트(`MatchRoom.ts`의 `DEFAULT_THROW_TIMEOUT_MS`/`DEFAULT_MOVE_TIMEOUT_MS`). 시간초과 시 서버가 §5와 동일한 확률 분포로 무작위 던지기 / 완주 안 한 첫 말을 자동 이동시킨다. 연결이 끊긴 플레이어의 턴도 이 타이머 덕분에 최대 20초 안에 자동으로 넘어가므로, 재접속 유예 중에도 게임이 멈추지 않는다.
  **윷/모 연속 던지기 버퍼(2026-09-03~)**: 윷/모(또는 잡기 보너스)로 다시 던질 기회가 생기면
  `GameBoard.tsx`가 결과 연출을 1.5초(`CHAIN_ANIM_MS`) 보여주며 그동안 다음 던지기 입력을 막는데,
  이 1.5초 동안에도 서버의 10초 카운트다운은 그대로 흐르고 있어서 손대지 않으면 실제로 누를 수
  있는 시간이 화면에 뜨는 10초보다 짧아진다 — "가끔씩 윷/모 직후에 자기 차례인데 던지지도 못하고
  넘어간다"는 신고의 원인 중 하나였다. `MatchRoom.ts`의 `armThrowTimeout`이 연속 던지기를 다시 걸
  때만 `CHAIN_ANIM_BUFFER_MS`(1500ms, `GameBoard.tsx`의 `CHAIN_ANIM_MS`와 반드시 같은 값 유지)를
  제한시간에 더해준다 — 둘 중 하나를 바꾸면 다른 쪽도 같이 맞출 것.
...
- **캐릭터(교주/성직/마담/의사) 능력 효과는 구현 완료**(2026-08-21~, `server/src/game/abilities.ts`) — 상세 규칙은 `docs/superpowers/specs/2026-08-21-character-abilities-design.md` 참고. 캐릭터 선택 개수는 모드별로 다름(2v2=서로 다른 2종, 1v1=중복 허용 4종, REQUIREMENTS.md §2).
  **"같은 줄" 판정에 꼭짓점/중앙/대각선 포함(2026-09-04~)** — 처음엔 outer 칸을 4개 변(A:1~5/
  B:6~10/C:11~15/D:16~20)으로만 나눠 "정확히 같은 변"이어야 성립했는데, 꼭짓점(5/10/15/20)이나
  중앙에 있는 말은 인접 라인·대각선까지 커버해야 자연스럽다는 사용자 지적으로 확장됨. 보드의
  두 대각선(5↔15, 10↔20, 둘 다 중앙을 지남)까지 포함해 한 칸이 여러 "줄"에 동시에 속할 수
  있음 — `position.ts`의 `sideOf(): Side | null`이 `sidesOf(): Side[]`로 바뀌었다(호출부는
  `abilities.ts`의 `sameSide` 하나뿐이라 영향 범위 작음). 마담/의사의 "같은 줄" 판정 전체가 이
  확장된 정의를 그대로 쓴다.
...
- **잡기와 업기 모두 `teamId` 기준이다(2026-09-02~)** — 예전엔 업기만 `ownerId` 기준(자신의 말끼리만)이었는데, 팀 동료의 말도 업힐 수 있어야 한다는 요청으로 `pieces.ts`의 `applyMove`를 `teamId` 비교로 바꿨다(REQUIREMENTS.md §6 v0.38). 교주 보너스의 "도착 칸에 이미 있던 아군 말" 그룹 판정(`MatchRoom.ts`)도 같은 이유로 `teamId` 기준으로 맞췄다. `Piece.teamId`는 `MatchRoom`이 매 이동마다 `players.get(ownerSessionId).team`을 조회해서 채우는 파생값이라, 연결이 끊긴 플레이어의 말은 팀 정보가 `""`로 빠질 수 있다(알려진 한계, 아래 참고) — 이 경우 팀 동료 업기도 함께 영향받을 수 있다.
- **승리 판정은 이번 이동을 실행한 사람만이 아니라 전원을 확인해야 한다(2026-09-06 버그 수정)** — 업기가 팀 기준으로 확장된 뒤(바로 위 항목), 내가 이동시킨 말에 팀 동료의 말이 업혀서 함께 완주할 수 있게 됐다. `performMove`의 승리 체크가 `checkWinner(finalPieces, sessionId)`처럼 이동을 실행한 sessionId 하나만 검사하고 있었는데, 그 결과 "동료의 말이 전부 완주됐지만 동료 자신의 턴이 아니라서" 승리가 감지되지 않는 버그가 있었다(신고 사례: 본인 말 2개+동료 말 1개=3개가 완주됐는데도 게임이 안 끝남). 더 나쁜 건, 그 동료는 이미 자기 말이 전부 완주해버려서 다음 자기 턴에 옮길 말이 없으므로(`autoMove`도 이동 대상이 없어 조용히 아무것도 안 함, 그 함수의 "자기 말이 모두 완주했다면 이미 승리 처리되어 턴이 없다"는 주석이 전제하는 불변조건이 이 경우 깨짐) 스스로 다시 승리 판정을 유발할 기회조차 없어 게임이 영원히 안 끝난다는 점이다. 고친 방식은 `[...this.state.players.keys()].find((pid) => checkWinner(finalPieces, pid))`로 이동 실행자뿐 아니라 방에 있는 모든 플레이어를 검사하는 것 — 팀 기준 업기가 존재하는 한, 승리 체크는 항상 "이번에 말이 움직인 모든 사람"이 아니라 "전체 플레이어"를 대상으로 해야 한다.

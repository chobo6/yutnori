...
describe("MatchRoom 전체 매치 흐름", () => {
  ...
  it(
    ...
    async () => {
      ...
      // 그 근처(elapsed≈0.97*cycleMs/2)를 노린다. 정확히 맞춰도 확인 확률(모/윷은 50%)에 실패하면
      ...
      for (let guard = 0; guard < 200; guard++) {
        ...
        if (isTarget) {
          await flush(291); // 정점 근처 -> "모" 구간 노림(목표 플레이어만 빠르게 전진)
        } else {
          await flush(30); // "도"(1칸) 구간 — 체인 없이 아주 느리게 전진(목표 말과 체크포인트 충돌 방지)
        }
